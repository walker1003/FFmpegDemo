...
// 把视频帧保存为PNG图片。save_index表示要把第几个视频帧保存为图片
int save_png_file(AVFrame* frame, int save_index) {
    ...
    char png_name[20] = { 0 };
    sprintf(png_name, "output_%03d.png", save_index);
    av_log(NULL, AV_LOG_INFO, "target image file is %s\n", png_name);

    enum AVPixelFormat target_format = AV_PIX_FMT_RGB24; // png的像素格式是RGB24
    // 分配图像转换器的实例，并分别指定来源和目标的宽度、高度、像素格式
    struct SwsContext* swsContext = sws_getContext(frame->width, frame->height, AV_PIX_FMT_YUV420P,
        frame->width, frame->height, target_format, SWS_FAST_BILINEAR, NULL, NULL, NULL);
    if (swsContext == NULL) {
        av_log(NULL, AV_LOG_ERROR, "swsContext is null\n");
        return -1;
    }
    AVFrame* rgb_frame = av_frame_alloc(); // 分配一个RGB数据帧。分配帧头部
    rgb_frame->format = target_format;  // 像素格式
    rgb_frame->width = frame->width;    // 视频宽度
    rgb_frame->height = frame->height;  // 视频高度
    // 分配缓冲区空间，用于存放转换后的图像数据。分配图像数据内存
    ...
    AVFormatContext* png_fmt_ctx = NULL;
    // 分配PNG文件的封装实例
    int ret = avformat_alloc_output_context2(&png_fmt_ctx, NULL, NULL, png_name);
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can't alloc output_file %s.\n", png_name);
        return -1;
    }
    // 查找PNG编码器
    AVCodec* png_codec = (AVCodec*)avcodec_find_encoder(AV_CODEC_ID_PNG);
    if (!png_codec) {
        av_log(NULL, AV_LOG_ERROR, "png_codec not found\n");
        return -1;
    }
    // 获取编解码器上下文信息
    ...
    png_encode_ctx->pix_fmt = target_format; // 像素格式
    png_encode_ctx->width = frame->width; // 视频宽度
    png_encode_ctx->height = frame->height; // 视频高度
    png_encode_ctx->time_base = (AVRational){ 1,25 }; // 时间基
    av_log(NULL, AV_LOG_INFO, "png codec_id = %d\n", png_encode_ctx->codec_id);
    ret = avcodec_open2(png_encode_ctx, png_codec, NULL); // 打开编码器的实例
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "Can't open jpg_encode_ctx.\n");
        return -1;
    }
    AVStream* png_stream = avformat_new_stream(png_fmt_ctx, 0); // 创建数据流
    ret = avformat_write_header(png_fmt_ctx, NULL); // 写文件头
    if (ret < 0) {
    ...
    while (ret == 0) {
        ...
        ret = av_write_frame(png_fmt_ctx, packet); // 往文件写入一个数据包
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "write frame occur error %d.\n", ret);
            break;
        }
    }
    av_write_trailer(png_fmt_ctx); // 写入文件尾
    av_frame_free(&rgb_frame); // 释放数据帧资源
    av_packet_free(&packet); // 释放数据包资源
    avio_close(png_fmt_ctx->pb); // 关闭输出流
    //avcodec_close(png_encode_ctx); // 关闭视频编码器的实例
    avcodec_free_context(&png_encode_ctx); // 释放视频编码器的实例
    avformat_free_context(png_fmt_ctx); // 释放封装器的实例
    return 0;
}
...
int decode_video(AVPacket* packet, AVFrame* frame, int save_index) {
    ...
    while (1) {
        ...
        save_png_file(frame, save_index); // 把视频帧保存为PNG图像
        break;
    }
    return ret;
}
...
int main(int argc, char** argv) {
    const char* src_name = "../resource/fuzhou.mp4";
    int save_index = 0;
    if (argc > 1) {
        src_name = argv[1];
    }
    ...
    av_log(NULL, AV_LOG_INFO, "Success save %d_index frame as png file.\n", save_index);
