...
int save_bmp_file(AVFrame* frame, int save_index) {
    ...
    // 转换器开始处理图像数据，把YUV图像转为BGR图像
    sws_scale(swsContext, (const uint8_t* const*)frame->data, frame->linesize, 0, frame->height, (uint8_t **)&out_buffer, linesize);
    ...
    uint8_t *tmp = (uint8_t *)av_malloc(frame->width * 3); // 临时数据
...
int decode_video(AVPacket* packet, AVFrame* frame, int save_index) {
    ...
    while (1) {
        ...
        save_bmp_file(frame, save_index); // 把视频帧保存为BMP图像
        break;
    }
    return ret;
}
