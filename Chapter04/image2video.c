...
AVCodecContext* image_decode_ctx[2] = { NULL, NULL }; // 视频解码器的实例
int video_index[2] = { -1, -1 }; // 视频流的索引
AVStream* src_video = NULL; // 源文件的视频流
AVStream* dest_video = NULL; // 目标文件的视频流
AVFormatContext* out_fmt_ctx = NULL; // 输出文件的封装器实例
...
int open_output_file(const char* dest_name) {
	...
	if (video_index[0] >= 0) { // 创建编码器实例和新的视频流
		...
		dest_video = avformat_new_stream(out_fmt_ctx, 0); // 创建数据流
		// 把编码器实例的参数复制给目标视频流
...
int recode_video(int seq, AVPacket* packet, AVFrame* frame) {
	...
	while (1) {
		...
		sws_scale(swsContext[seq], (const uint8_t* const*)frame->data, frame->linesize, 0, frame->height,
				  yuv_frame[seq]->data, yuv_frame[seq]->linesize);
		int i = 0;
...
int main(int argc, char** argv) {
	const char* src_name0 = "../resource/input.jpg";
	const char* src_name1 = "../resource/input.png";
	const char* dest_name = "../resource/output_image2video.mp4";
	if (argc > 1) {
		src_name0 = argv[1];
	}
	if (argc > 2) {
