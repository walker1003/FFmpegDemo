...
int save_jpg_file(AVFrame* frame, int save_index) {
    ...
    AVFrame* yuvj_frame = av_frame_alloc(); // 分配一个YUVJ数据帧。分配帧头部
    ...
    av_image_alloc(yuvj_frame->data, yuvj_frame->linesize, frame->width, frame->height, target_format, 1);
    // 转换器开始处理图像数据，把YUV图像转为YUVJ图像
    sws_scale(swsContext, (const uint8_t* const*)frame->data, frame->linesize, 0, frame->height, yuvj_frame->data, yuvj_frame->linesize);
...
int decode_video(AVPacket* packet, AVFrame* frame, int save_index) {
    ...
    while (1) {
        ...
        save_jpg_file(frame, save_index); // 把视频帧保存为JPEG图像
        break;
    }
    return ret;
}
...
int main(int argc, char** argv) {
    const char* src_name = "../resource/fuzhous.mp4";
    int save_index = 0;
    if (argc > 1) {
        src_name = argv[1];
    }
