...
int main(int argc, char** argv) {
	...
	const char* dest_name = "../resource/output_splitvideo.mp4";
	if (argc > 1) {
		src_name = argv[1];
	}
	if (argc > 2) {
	...
	av_log(NULL, AV_LOG_INFO, "begin_video_pts=%d, end_video_pts=%d\n", begin_video_pts, end_video_pts);
	// 寻找指定时间戳的关键帧，并非begin_video_pts所处的精确位置，而是离begin_video_pts最近的关键帧(往回往小的方向找)
	ret = av_seek_frame(in_fmt_ctx, video_index, begin_video_pts, AVSEEK_FLAG_FRAME | AVSEEK_FLAG_BACKWARD);
	if (ret < 0) {
	...
	while (av_read_frame(in_fmt_ctx, packet) >= 0) { // 轮询数据包
		if (packet->stream_index == video_index) { // 为视频流
			...
			// av_log(NULL, AV_LOG_INFO, "packet->pts=%d, packet->dts=%d\n", packet->pts, packet->dts);
			packet->pts = packet->pts - key_frame_pts; // 调整视频包的播放时间戳
			packet->dts = packet->dts - key_frame_pts; // 调整视频包的解码时间戳(关键帧的pts=dts)
	...
	av_log(NULL, AV_LOG_INFO, "Success split video.\n");
