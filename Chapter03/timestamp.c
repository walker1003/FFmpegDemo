...
int main(int argc, char** argv) {
	...
	if (video_index >= 0) {
		...
		av_log(NULL, AV_LOG_INFO, "video_timestamp increment=%d\n", timestamp_increment);
	}
	...
	if (audio_index >= 0) {
		...
		/*
		1. time_base = 1/44100:
		   增量 = 1024 * (44100 / 44100) ＝ 1024
		2. time_base = 1/90000:
		   增量 = 1024 * (90000 / 44100) ≈ 2089.79
		   在 FFmpeg 中，时间戳必须是整数。则会发现每一帧的增量可能在 2089 或 2090 之间跳动，
		   以确保长期运行下的时间误差不累积。
		3. time_base.den 表示时间刻度，sample_rate 表示一秒中有多少个样本，frame_size 表示一帧有多少样本。
		*/
	}
