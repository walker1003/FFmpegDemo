...
int open_input_file(const char* src_name) {
    ...
    //av_dist_set(&options, "ignore_editlist", "1", 0);
    //av_dist_set(&options, "scan_all_pmts", "1", 0);
    //int ret = avformat_open_input(&in_fmt_ctx, src_name, NULL, &options);
    // 打开音视频文件
...
int output_video(AVFrame* frame) {
    ...
    while (1) {
        ...
        //av_log(NULL, AV_LOG_INFO, "pts=%ld, dts=%ld.\n", packet->pts, packet->dts);
...
int main(int argc, char** argv) {
    ...
    const char* dest_name = "../resource/output_recode2.mp4";
    if (argc > 1) {
        src_name = argv[1];
    }
    if (argc > 2) {
    ...
    av_log(NULL, AV_LOG_INFO, "Success recode file.\n");
