...
int main(int argc, char** argv) {
    const char* video_name = "../resource/fuzhous.mp4";
    const char* audio_name = "../resource/fuzhous.aac";
    const char* dest_name = "../resource/output_mergeaudio.mp4";
    if (argc > 1) {
        video_name = argv[1];
    }
    if (argc > 2) {
    ...
    if (video_index >= 0) { // 源文件有视频流，就给目标文件创建视频流
        ...
        dest_video->codecpar->codec_tag = 0; // 自动生成视频编码器标签(不是编码器ID)
    }
    ...
    if (audio_index >= 0) { // 源文件有音频流，就给目标文件创建音频流
        ...
        dest_audio->codecpar->codec_tag = 0; // 自动生成音频编码器标签(不是编码器ID)
    }
    ...
    while (1) {
        ...
        if (last_video_pts == 0 || av_compare_ts(last_video_pts, dest_video->time_base, last_audio_pts, dest_audio->time_base) <= 0) {
            // 轮询视频包
            while ((ret = av_read_frame(video_fmt_ctx, packet)) >= 0) {
...
//av_packet_rescale_ts(packet, 旧时间基, 新时间基)
//自动修改 pts、dts、duration
//从旧时间基 → 转到新时间基
//不会修改时间基本身
//时间基属于流，不属于包