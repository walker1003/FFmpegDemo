...
int main(int argc, char** argv) {
	const char* filename = "../resource/output.mp4";
	if (argc > 1) {
		filename = argv[1];
	}
	...
	AVCodecContext* video_encodec_ctx = NULL;
	// 分配编解码器的实例
	video_encodec_ctx = avcodec_alloc_context3(video_codec);
	if (!video_encodec_ctx) {
		av_log(NULL, AV_LOG_ERROR, "video_encode_ctx is null\n");
		return -1;
	}

	video_encodec_ctx->width = 320; // 视频画面的宽度
	video_encodec_ctx->height = 240;// 视频画面的高度
	// 创建指定编码器的数据流
	AVStream* video_stream = avformat_new_stream(out_fmt_ctx, video_codec);
	// 把编码器实例中的参数复制给数据流
	avcodec_parameters_from_context(video_stream->codecpar, video_encodec_ctx);
	video_stream->codecpar->codec_tag = 0; // 非特殊情况都填0

	ret = avformat_write_header(out_fmt_ctx, NULL); // 写文件头
	if (ret < 0) {
		av_log(NULL, AV_LOG_ERROR, "write file_head occur error %d.\n", ret);
		return -1;
	}
