...
int main(int argc, char** argv) {
	...
	if (ret < 0) {
		av_log(NULL, AV_LOG_INFO, "Can't find stream information.\n", filename);
		return -1;
	}
	...
	av_log(NULL, AV_LOG_INFO, "video_codec_id=%d\n", fmt_ctx->video_codec_id); // 配置参数。在打开文件前，可以强制指定使用哪个解码器（通常用于那些没有头部信息、无法自动探测格式的原始流，如裸流）
	av_log(NULL, AV_LOG_INFO, "audio_codec_id=%d\n", fmt_ctx->audio_codec_id); // 由于一个多媒体文件可能包含多个视频流（比如不同角度）或多个音频流（比如不同语言），在AVFormatContext里放一个单一的video_codec_id是无法表示所有流的。因此，真正的结果必须去每个流（AVStream）里查找
