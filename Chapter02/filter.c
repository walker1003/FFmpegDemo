...
int init_filter(AVStream* video_stream, AVCodecContext* video_decode_ctx, const char* filters_desc) {
	...
	if (ret < 0) {
		av_log(NULL, AV_LOG_ERROR, "Can't parse graph string\n");
		return ret;
	}
	...
	if (ret < 0) {
		av_log(NULL, AV_LOG_ERROR, "Can't config filter graph\n");
		return ret;
	}
...
int main(int argc, char** argv) {
	...
	avformat_close_input(&fmt_ctx); // 关闭音视频上下文
	avformat_free_context(fmt_ctx); // 释放音视频上下文
	return 0;
}
